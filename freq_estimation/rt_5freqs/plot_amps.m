...
hamps = [hamps(:, 6) hamps(:, 3) hamps(:, 5) hamps(:, 7) hamps(:, 4)];
fft_hamps = [fft_hamps(6) fft_hamps(3) fft_hamps(5) fft_hamps(7) fft_hamps(4)];
...
grid on;
hold on;
plot(0:size(hamps, 1), zeros(size(hamps, 1) + 1, ...
    size(hamps, 2) + 1) + [bias amps]);
